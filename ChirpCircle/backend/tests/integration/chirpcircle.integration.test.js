// tests/integration/chirpcircle.integration.test.js
...
describe('ChirpCircle End-to-End Tests (TC 1–20)', () => {
  beforeAll(async () => {
    ...
    const olgaRes = await request(app)
    ...
    const maryRes = await request(app)
      .post('/api/v1/auth/local-test-login')
      .send({ username: 'Mary' })
      .expect(200);
    maryAccessToken = maryRes.body.data.accessToken;

    const nestorRes = await request(app)
      .post('/api/v1/auth/local-test-login')
  ...
  test('TC 4: Olga posts a Tech message', async () => {
    ...
    const res = await request(app)
      ...
      .send({
        content: 'Olga’s Tech Post',
        topic: 'Tech',
        expiration_time: expirationTime,
      })
      .expect(201);

    // Access the data object now
